...
public class ExchangeServiceImpl implements ExchangeService {
    ...
    public BigDecimal getPriceInCurrency(BigDecimal price, String currency) {
        ...
        switch (currencyEnum){
            case CZK:
                newPrice = price.multiply(BigDecimal.valueOf(24.31)).setScale(2, RoundingMode.HALF_EVEN);
                break;
            case EGP:
                newPrice = price.multiply(BigDecimal.valueOf(18.89)).setScale(2, RoundingMode.HALF_EVEN);
                break;
            case GBP:
